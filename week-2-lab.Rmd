...
2.  Discuss whether these two variables appear correlated, what sign you anticipate the correlation to be, and the strength of the correlation you expected.
...
# These are correlated positively, but not very strong.
```
...
# These are strongly negatively correlated.
```
...
```{r}
cor(x = pie_crab$latitude, y = pie_crab$size)

cor(x = pie_crab$latitude, y = pie_crab$water_temp)
```
...
1.  Use `lm()` to estimate $\hat\beta_0$ and $\hat\beta_1$ using this sample of data. Use `summary(lm())` or `gt()` to visualize the regression results.
    ...
    Used lm and ordinary least squares to get the beta hat 0 and beta hat 1 (from sample).

2.  Interpret your two coefficients, paying careful attention to units.

    The intercept estimate is -3.62, which is what the size would hypothetically be when x is at 0. Latitude is the slope of the line and dependent on the units. We're assuming that the size is measured in cm.

3.  Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$?

    No, it is not directly interpretable because there can be no negative size.
...
# The se adds a grey shaded confidence interval.
```
...
```{r}
crab_lm <- lm(size ~ latitude, data = pie_crab)
pie_crab2 <- pie_crab
  
pie_crab2 <- pie_crab2 %>% 
  mutate(size_residual = resid(crab_lm),
         predicted_size = predict(crab_lm))
...
```{r}
ggplot(pie_crab2, aes(x = size_residual)) +
  geom_histogram()
```
...
```{r}
ggplot(pie_crab2, aes(x = latitude, y = size_residual)) +
  geom_point() +
