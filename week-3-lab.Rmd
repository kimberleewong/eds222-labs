...
```{r}
summary(lm(pie_crab$size ~ pie_crab$latitude))
# R^2 = 0.348

summary(lm(pie_crab$water_temp ~ pie_crab$latitude))
# R^2 = 0.916
```
...
```{r}
residuals <- resid(lm(size ~ latitude, data = pie_crab))
...
mpg <- mpg %>% 
  mutate(year_fct = factor(mpg$year))
...
ggplot(mpg, aes(x = hwy, color= year_fct)) +
...
1.  Using the model specified above, use `lm()` to estimate $\hat\beta_0$ and $\hat\beta_1$ using this sample of data. Make sure you are treating `year` as a categorical variable! Use `summary(lm())`, `gt()` or `kable()` to visualize the regression results.
    ...
    ```{r}
    summary(lm(hwy ~ year_fct, data = mpg))
...
2.  What does the intercept tell you, in words?
    ...
    The intercept says that when the car is from 1999, the average mpg on the highway is 23.42.

3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? **Answers:**

    The coefficient estimate on the 2008 vintage indicator tells us that on average the highway mpg for 2008 cars is higher than 1999 by 0.0256.

<!-- -->

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

    The model does not really suggest that fuel efficiency has evolved over time.
